...
const getLoopStartLengthAndOffset = ({
  offsetFactor,
  i,
  loopIndexes,
  loopsLength,
...
class SpiralCanvas extends React.PureComponent {
  ...
  animate = (loopsInfo, count, onComplete) => {
    if (count <= 100) {
      ...
      requestAnimationFrame(() => {
        this.animate(loopsInfo, count + 2, onComplete);
      });
    } else {
      onComplete();
    }
  ...
  componentDidMount() {
    ...
    if (animate) {
      if (!editing) {
        ...
        // Start the animation
        requestAnimationFrame(() => {
          this.animate(loopsInfo, 0, onComplete);
        });
        onStartAnimation();
      }
    } else {
  ...
  updateCanvas = (loopsInfo, count) => {
    ...
    if (inner && inner.length) {
      ...
      if (loopsInfo && count !== 100) {
        // Don't want to animate last loop because of strokes
  ...
  render() {
    ...
    return [
      <Canvas
        key="canvas"
        className={type === "print" ? "downloadable" : ""}
        highlight={highlight}
        accent={accent}
        active={active}
      ...
      interactive && (
        <Overlay
          ...
          key="overlay"
        >
          <WorkspaceIconAndText active text={`Click to crop`} type="move" />
        </Overlay>
      ),
    ];
  }
