...
class App extends Component {
  ...
  setSliderActive = (val) => {
    this.setState({ sliderActive: val });
  };
  setEditingPhoto = (val) => {
  ...
  setColorIndex = (colorIndex) => {
    this.setState(({ filter: prevFilter }) => {
      return { filter: { ...prevFilter, colorIndex } };
    });
  };
  setPreview = (preview) => {
    this.setState({ preview });
  };
  setAttributeChange = (att) => {
    this.setState({ attribute: att });
  };
  setClickedDownload = (val) => {
    this.setState({ clickedDownload: val });
  };
  setAnimating = (val) => {
      ...
      getImageData(url, orientation).then(
        ({ status, width, height, imgData: data }) => {
          if (status === "ok") {
            ...
            alert(
              "Sorry, we could not load image data for this file. Check to make sure you are using JPG, GIF, PNG, or WebP."
            );
          }
        }
      );
  ...
  handleMultipleFiles = (files) => {
    ...
    ).then((files) => this.setImagesData(files.filter((f) => !!f)));
  };
  handleFileChange = (e) => {
  ...
  downloadCanvas = () => {
    ...
    const processes = Array.from(
      document.querySelectorAll("canvas.downloadable")
    ).map((canvas) => {
      return new Promise((rs, rj) => {
        canvas.toBlob(
          function (blob) {
            const dt = new Date();
            zip.file(`spiralbetty_${dt.getTime()}.jpg`, blob);
            rs(true);
          },
          "image/jpeg",
          0.95
    ...
    Promise.all(processes).then(() => {
      zip
        ...
        .then((blob) => FileSaver.saveAs(blob, `images.zip`));
    });
  ...
  checkToSeeIfCanvasForDownloadExists = () => {
    ...
    this.checkCanvasDownoad = setInterval(() => {
      if (count > maxAttempts) {
        ...
        return;
      } else if (document.querySelector("canvas.downloadable")) {
        this.downloadCanvas();
        clearInterval(this.checkCanvasDownoad);
        this.checkCanvasDownoad = null;
        return;
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevState.clickedDownload === false &&
      this.state.clickedDownload === true
    ) {
  ...
  render() {
    ...
    // TODO: Splash page
    return (
      <AppContainer darken={editing} style={{ height: winHeight }}>
        <Beforeunload
          onBeforeunload={() =>
            "Sure you want to leave this site? You will lose your progress"
          }
        />
        <MobileHeader
          ...
          download={() => {
            this.setClickedDownload(true);
          }}
          removeImg={this.removeImg}
        />
        <MobileMargin style={{ flex: `0 0 ${winHeight * 0.05}px` }} />
        <Main id="main">
                  ...
                  {!editing && !animating && (
                    ...
                    >
                      ...
                      <NavLinks
                        ...
                        onClick={() => {
                          this.setAttributeChange("lightness");
                        }}
                      >
                        Lightness
                      </NavLinks>
                      <NavLinks
                        ...
                        onClick={() => {
                          this.setAttributeChange("contrast");
                        }}
                      >
                        Contrast
                      </NavLinks>
